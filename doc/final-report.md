...
最终方案不是再训练一个新的 checkpoint，而是以固定的 `checkpoint_step_3000` 为判定核心，并在其前后配置可审计、可离线复现的防护策略。它将“明显攻击先快速阻断”“模糊样本交给多模态模型”“按已验证偏差校准分数”“高置信安全样本快速放行”拆成不同责任层，避免把所有问题都交给单一模型承担。

#### 2.1.1 从输入到决策的运行流水线

下图描述逻辑执行顺序。MCR 和 SBC 分别是分类器调用的输入构造和 logits 后处理，逻辑上位于 F-3000 判定内部；C5、C6B-lite、C6A 和 C4 则是 pipeline 的前后置关卡。
...
flowchart TD
    ...
    subgraph H["分类器调用与校准"]
        ...
        F --> S["SBC：固定类别分数校准<br/>Indirect +0.55；MCR 时 Direct -0.20"]
    end
...
| MPID 三分类头 | 分类头将隐藏表征映射为 `clean/direct/indirect` 三个 logits。显式保留攻击来源类别，才能针对文本直接注入和图像/间接注入学习不同边界。 | 固定使用 `checkpoint_step_3000.safetensors`，处理规则无法确认的语义边界样本，并输出 C4 与 SBC 所需的概率分布；单独使用时仍受训练分布和类别偏置影响。 |
...
| SBC：范围化偏差校准 | 分类器 logits 未必与真实类别概率对齐。类别条件的固定偏移可在不修改模型权重的情况下移动决策边界，是将训练后观察到的系统性偏差显式化的后验校准方法。 | 依据 smoke 结果锁定：所有样本的 `Indirect` logits 加 `0.55`；MCR 启用时 `Direct` logits 减 `0.20`。它针对图像上下文开启后仍偏向 direct 的偏差；系数必须固定并在盲测集验证，不能针对测试样本临时调参。 |
| C4：高置信 clean 门 | 这是一种选择性分类思想：只在模型对安全类别足够确定时走快速放行路径，低置信样本保持更保守的最终分类处理。阈值将便利性与漏检风险显式化。 | 在 C5/C6 风险关卡和校准后检查 `P(clean) > 0.95`，满足则以 `c4_early_exit` 放行，减少后续生成或业务处理；否则按校准后的类别决定。它不会绕过任何已命中的风险规则，也不将低置信样本默认为安全。 |
...
完整横评指标、训练配置、逐类预测和审计材料见 [reference.md](/C:/work/llm-compliance/doc/reference.md:3557) 第三章及 `runs/phase2_3_full_3000_20260729_0958/artifacts/`。
