...
- [1. 核心概念](#1-核心概念)
  - [1.1 威胁模型（Threat Model）](#11-威胁模型threat-model)
  - [1.2 数据集构造（Dataset Construction）](#12-数据集构造dataset-construction)
  - [1.3 EDA（探索性数据分析）](#13-eda探索性数据分析)
  - [1.4 Macro F1 完全指南](#14-macro-f1-完全指南)
  - [1.5 LoRA 原理与使用技巧](#15-lora-原理与使用技巧)
- [2. 项目实施 Phase 详解](#2-项目实施-phase-详解)
  - [当前运行目录约定](#当前运行目录约定)
  ...
  - [Phase 2.5 — 成果可视化 Demo](#phase-25--成果可视化-demo220-226)
...
- [4. 未来展望](#4-未来展望)
  ...
  - [4.3 为什么 LoRA 故意只挂语言侧（深度版）](#43-为什么-lora-故意只挂语言侧深度版)
  - [4.4 未来可继续做的工作（3 阶段路线图）](#44-未来可继续做的工作3-阶段路线图)
  - [4.5 预期达到的效果（量化指标）](#45-预期达到的效果量化指标)
  - [4.6 与开题报告研究目标的对应](#46-与开题报告研究目标的对应)
  - [4.7 答辩常问 Q&A 预演](#47-答辩常问-qa-预演)
  - [4.8 一句话总结](#48-一句话总结)
  - [4.9 项目交付物深度解析：给第三方的不仅是模型](#49-项目交付物深度解析给第三方的不仅是模型)
- [5. 实施问题汇总](#5-实施问题汇总)
  - [5.1 本节用途](#51-本节用途)
  - [5.2 问题总览](#52-问题总览)
  - [5.3 典型问题详解](#53-典型问题详解)
  - [5.4 从问题中沉淀出的工程改动](#54-从问题中沉淀出的工程改动)
  - [5.5 给后来执行者的建议](#55-给后来执行者的建议)
- [6. 附录](#6-附录)
  - [6.A 项目术语速查](#6a-项目术语速查)
  - [6.B 行业术语速查](#6b-行业术语速查)
  - [6.C 速查卡片](#6c-速查卡片)
  - [6.D 引用](#6d-引用)
...
> **本部分解释项目里五个核心概念**：威胁模型、数据集构造、EDA、Macro F1、LoRA。每个概念按"是什么 / 为什么 / 怎么落地到本项目"三段式说明。
>
> **本部分由原独立文件 `threat_model.md`（C1 产出）合并而来**——所有威胁模型相关的内容（攻击分类、形式化定义、攻击者能力假设、防御目标、三层防御对应）都集中在此。原来的 `threat_model.md` 已被删除，所有内容并入本文件。
>
> **本部分与第二部分的关系**：第二部分讲"按什么顺序做"，本部分讲"做的过程中碰到的概念是什么"——开发时碰到不熟悉的术语可以快速回查。
...
### 1.2 数据集构造（Dataset Construction）
...
- `runs/<run_id>/` 表示一次隔离的端到端执行。具体 run id 必须带时间后缀，例如 `phase2_2_balanced_600_<timestamp>`。
...
| [reference.md § 1.1](reference.md#11-威胁模型threat-model) | 威胁模型形式化定义 | T1.2 |
...
| 评测项 | 目的 | 典型对比 |
|---|---|---|
| `MPID LoRA` | Phase 2.2 checkpoint 的原始能力 | LoRA + head，不启用 C4/C5/C6 |
| `C4 + MPID LoRA` | 衡量早退是否节省延迟且不伤害安全性 | `P(clean)>θ` 时提前 allow |
| `C5 + MPID LoRA` | 衡量规则前置是否提升 direct recall / 降低成本 | direct 规则命中直接 block，未命中交给 MPID LoRA |
| `C6 + MPID LoRA` | 衡量跨模态自检是否提升 indirect recall | figstep / 图文冲突 / OCR 信号命中 block |
| `C4 + C5 + C6 + MPID LoRA` | 衡量完整 defense-in-depth 系统收益 | 真实部署路径 |
...
| `stage distribution` | 看每条样本停在哪一层：C4/C5/C6/MPID LoRA |
...
```text
模型能力 = MPID LoRA 能否判断 clean/direct/indirect
...
| T2.16 | `runs/<run_id>/scripts/launch.ps1`（Windows）或 `runs/<run_id>/scripts/launch.sh`（macOS / Linux）+ `trainer.py`（加 `save_every` / signal handler） | 按平台后台启动训练 + 进度保护 |
...
```powershell
# Windows PowerShell：推荐使用 run-local launcher
powershell -ExecutionPolicy Bypass -File .\runs\<run_id>\scripts\launch.ps1
```

```bash
# macOS / Linux：继续使用 shell 脚本入口
bash runs/<run_id>/scripts/launch.sh
...
- Windows PowerShell 版本会把每一步的 stdout / stderr 归档到 `runs/<run_id>/logs/`
...
#### 补充：通用 balanced/full 训练复现模板

上面 5 步是 **Phase 2.2 的最小验收链路**。如果团队成员要重新训练一个可用于后续 Phase 3-5 的 checkpoint，应使用新的 run 目录和对应配置，而不是复用历史 run id。

```bash
# 1) 训练：配置文件由本次 run 固定，checkpoint 名称由配置或参数决定
python -X utf8 -u scripts/train.py \
  --config runs/<run_id>/configs/train.yaml \
  --preload-dataset \
  --save-every 100 \
  --max-train-seconds <seconds_budget> \
  --checkpoint-name <checkpoint_name>.safetensors \
  --partial-name <checkpoint_name>.safetensors

# 2) 单模型评估
python -X utf8 scripts/eval.py \
  --config runs/<run_id>/configs/train.yaml \
  --checkpoint runs/<run_id>/artifacts/checkpoints/<checkpoint_name>.safetensors \
  --out runs/<run_id>/artifacts/eval

# 3) 与 smoke baseline 对比
python -X utf8 scripts/eval.py \
  --config runs/<run_id>/configs/train.yaml \
  --compare-smoke-vs-full \
  --smoke-checkpoint runs/<run_id>/artifacts/checkpoints/lora_baseline.safetensors \
  --full-checkpoint runs/<run_id>/artifacts/checkpoints/<checkpoint_name>.safetensors \
  --out runs/<run_id>/artifacts/comparison

# 4) 重新打离线包并冒烟
python -X utf8 scripts/package_offline.py \
  --ckpt runs/<run_id>/artifacts/checkpoints/<checkpoint_name>.safetensors \
  --out runs/<run_id>/artifacts/package/mpid_offline
...
python -X utf8 -u scripts/train.py \
  --config runs/<run_id>/configs/train.yaml \
  --preload-dataset \
  --resume-from runs/<run_id>/artifacts/checkpoints/<checkpoint_name>.safetensors \
...
本 reference 以 V1 为准；V2 只作为后续扩展路线，不应混入当前验收口径。
...
#### [pipeline.py](../src/mpid/infer/pipeline.py) 的 `run_lightweight_pipeline`
Phase 3 后面新增的轻量流水线，把 C4 放到了一个真正可串联 C5/C6 的调度器里。它的价值不在于替代真实 VLM 推理，而在于先把阶段输出契约固定下来：

```python
def run_lightweight_pipeline(record, *, probs=None, clean_threshold=0.95):
...
这个片段说明了 3 个设计点：
1. **C4 只消费概率，不关心概率来自哪里**：真实路径里它来自 VLM + head；smoke 路径里可以手工注入 `probs=[...]`。
2. **C4 的输出不是裸 label，而是完整 decision object**：必须同时返回 `label / action / stage / explanation`，这样后面的 C5/C6 和 Phase 6 消融评测才能共用同一套 schema。
3. **C4 命中后立即短路返回**：这就是“早退”的本质，后续规则和跨模态检查都不再执行。

这也是为什么 `tests/test_pipeline_lightweight.py` 里专门有一条看起来反直觉的用例：即使文本里写了 `Ignore previous instructions.`，只要传入的 `P(clean)` 高到超过阈值，流水线依然会在 `c4_early_exit` 结束。这个测试不是说这种配置安全，而是在验证调度优先级是否和设计一致。
...
#### [engine.py](../src/mpid/rules/engine.py) 的 `RuleMatch / RuleResult / scan_text`
C5 轻量规则层的核心不是“关键词表”本身，而是把命中证据结构化。这也是它和简单 `if "ignore previous" in text` 最大的区别。
...
这里的设计意图很明确：
1. **`RuleMatch` 表示单条证据**：例如 `direct.ignore_previous` 或 `sensitive.malware`。
2. **`RuleResult` 表示最终决策**：一个样本可能同时命中多条规则，但最终只输出一次 `block/pass`。
3. **结果对象天然可序列化**：`to_dict()` 直接进入 report、pipeline explanation 和离线审计日志。

真正的检测逻辑在 `scan_text()`：

```python
matches.extend(_scan_patterns(value, _DIRECT_PATTERNS, category="keyword", label="direct", severity="high"))
matches.extend(_scan_patterns(value, _SENSITIVE_PATTERNS, category="sensitive", label="direct", severity="medium"))
matches.extend(_scan_patterns(value, _STRUCTURE_PATTERNS, category="structure", label="direct", severity="medium"))
matches.extend(_scan_patterns(value, _UNICODE_PATTERNS, category="unicode", label="direct", severity="medium"))
...
这段代码背后有 4 个关键工程判断：
1. **规则按组分层，而不是混成一坨正则**：便于以后统计“是 direct 模板命中多，还是 Unicode 混淆命中多”。
2. **命中即 block，未命中只是 pass-through**：C5 不负责最终放行，它只是前置拦截层。
3. **当前是保守 OR 逻辑**：只要某个高确定性模式命中，就直接判为 `direct`。
4. **snippet 被强制截短**：既保留审计证据，又不把整段原文原样复制进日志。

与之对应的单测也正好覆盖了三层目标：
- `test_scan_text_clean_passes()` 验证干净文本不会误杀。
- `test_scan_text_direct_injection_blocks()` 验证典型模板攻击一定能命中。
- `test_scan_text_sensitive_blocks_with_explanation()` 验证 block 时一定保留 explainable snippet。

#### [pipeline.py](../src/mpid/infer/pipeline.py) 的 C5 接入点
C5 真正接入系统，不是靠 `eval_rules.py`，而是靠轻量调度器里的这一段：
...
这里有两个特别重要的点：
1. **C5 的输入只有文本**：这保证它完全无状态、低成本、可热更新，不依赖模型和图片。
2. **C5 的解释直接复用规则结果对象**：后面 Phase 6 统计 `stage distribution`、`explanation coverage` 时，不需要再写一套格式转换逻辑。

从系统角度看，C5 不是“又一个分类器”，而是一个可以被调度器短路的确定性拦截器。它命中时节省的不是一点点 Python 时间，而是整个后续 C6 检查和 VLM fallback 的成本。
...
| direct 覆盖 | smoke 中 direct recall 有明显正收益；完整验证时与 MPID LoRA 对比 |
...
#### [heuristic.py](../src/mpid/crossmodal/heuristic.py) 的 `CrossModalResult / check_crossmodal`
C6A 当前是“轻量启发式版”，所以它的关键不是图像理解深度，而是先把跨模态判定的输入/输出契约固定下来。
...
def check_crossmodal(record=None, *, text=None, image=None) -> CrossModalResult:
    rec = record or {}
...
这里的设计很像 C5，但语义不同：
1. **`CrossModalResult` 不是 rule hit 列表，而是风险信号集合**：C6 更关心“为什么怀疑它是 indirect”。
2. **函数既能吃完整 record，也能单独吃 `text/image`**：这让它既能跑在 JSONL evaluator 里，也能被 pipeline 直接调用。
3. **输出里保留 `reasons`**：为后续接 OCR / CLIP / VLM 自问版本留统一接口。

当前 C6A 的核心判定只有 3 组信号：
...
这段代码体现了当前 C6A 的真实定位：
1. **先证明 indirect 管线是可跑的**，而不是假装已经做完 OCR/CLIP。
2. **优先抓 FigStep 家族**，因为它是现有数据里最明确、最稳定的跨模态攻击来源。
3. **把“图片路径声明存在但本地不可读”也记成风险信号**：这既是数据完整性问题，也是离线部署时值得提示的异常状态。

对应单测也正好覆盖了三个层次：
- `test_clean_text_not_suspicious()`：干净样本不应乱报。
- `test_figstep_metadata_marks_indirect()`：metadata/path 命中应能直接拦截。
- `test_figstep_text_pattern_marks_indirect()`：即使图片字段缺失，文本模式仍可触发轻量 C6A。

#### [pipeline.py](../src/mpid/infer/pipeline.py) 的 C6 接入点
C6 在系统里的位置，体现在调度器的这一段：
...
这里最值得注意的是顺序关系：
1. **C6 只有在 C4 没放行、C5 没拦截时才运行**，所以它天然是“兜底层”。
2. **C6 命中后直接 block，不再进入 fallback**，说明它和 C5 一样属于决策层，而不是仅供参考的旁路信号。
3. **未命中时不会擅自判 clean**，而是明确返回 `defer_to_vlm`。这保证轻量启发式不会越权替代 Phase 2 的模型分类器。

`tests/test_pipeline_lightweight.py` 里专门有一条 `test_pipeline_c6_blocks_figstep_when_rules_do_not_hit()`，它验证的正是这一层级关系：文本规则没命中时，C6 仍然能靠 FigStep 信号在 `stage="c6_crossmodal"` 处拦下样本。
...
- 用固定 checkpoint 分别跑 `MPID LoRA`、`C4+MPID LoRA`、`C4+C5+MPID LoRA`、`C4+C5+C6+MPID LoRA`。
...
**用统一 test set、统一指标、统一输出 schema，对 `MPID LoRA`、C4、C5、C6 和外部/简单基线做可复现对比，形成项目最终能力结论。**
...
| `MPID LoRA` | Phase 2.2 checkpoint 原始能力 | 三分类报告 + 混淆矩阵 |
| `C4 + MPID LoRA` | 早退收益与风险 | exit_rate / wrong_exit / saved_pct |
| `C5 + MPID LoRA` | direct 规则前置收益 | rule_hit_rate / direct_recall / clean_FPR |
| `C6 + MPID LoRA` | indirect 跨模态收益 | indirect_recall / reasons 分布 |
| `C4 + C5 + C6 + MPID LoRA` | 最终部署路径 | 综合 F1 / latency / stage distribution |
...
|---|---:|---:|---:|---:|---:|---|
| MPID LoRA | 待填 | 待填 | 待填 | 待填 | 待填 | Phase 2.2 baseline |
| C4+MPID LoRA | 待填 | 待填 | 待填 | 待填 | 待填 | 看 wrong_exit |
| C5+MPID LoRA | 待填 | 待填 | 待填 | 待填 | 待填 | 看规则误杀 |
| C6+MPID LoRA | 待填 | 待填 | 待填 | 待填 | 待填 | 看 indirect |
...
| 项 | 通过条件 |
|---|---|
| 消融完整性 | 至少包含 `MPID LoRA`、`C4`、`C5`、`C6A`、`Full pipeline` |
...
| **Full-3000，F-3000-MCR-SBC 离线全链路** | **V2/full** | **500** | **62.0%** | **59.28%** | **最终正式验收** | **三类 recall 非零并通过 45% Macro F1 / 35% Direct F1 门槛；当前推荐交付方案** |

**总览判断。** V1 的结果用于记录历史模型与完整防御链路的正式表现；V2/smoke 的 A-H 用于在完全相同的数据、评测脚本与 batch-size 下消除历史横比偏差。最终候选必须以 V2/full 的 500 条端到端预测验收。本轮唯一满足该要求的是 `F-3000-MCR-SBC`：它是 `checkpoint_step_3000` 与冻结推理策略组成的完整离线 pipeline，不是新的训练 checkpoint。
...
### 3.4 Standard Benchmark v2：Full-3000 策略筛选与最终验收
...
#### 3.4.2 F 正式 Full-3000 训练与尝试记录

正式训练以 F 为基线，使用 3,000 条均衡训练样本（每类 1,000 条）、配对三元组 batch、SmolVLM-500M 和 LoRA `r=32 / alpha=64 / dropout=0.10`。训练使用 `batch_size=3`、`paired_batch_mode=true`、`direct_margin=0.35`、每 50 step 保存 checkpoint 与 optimizer/RNG sidecar，并按阶段 checkpoint 恢复至 `checkpoint_step_3000`。训练过程无 NaN、Inf 或 traceback，最终 checkpoint 与 state 文件均可读取。

| 尝试 | 是否训练 | 选择方式与结论 | V2/full500 结果或处理 |
|---|---|---|---|
| Step 3000 原始 head | 是 | 原始 LoRA + 分类 head；Indirect 完全塌缩 | Accuracy 56.60%，Macro F1 38.89%，Clean/Direct/Indirect F1=67.19%/49.48%/0.00% |
| R0 | 否 | 固定 global Indirect logit offset `+0.55`；后续不得重新搜索 | Macro F1 41.23%，Indirect F1 8.93%；仅恢复极少量 Indirect，不满足最终门槛 |
| logits ensemble | 否 | Step2250/3000 已保存 logits 组合 | 未改善三类平衡，保留为失败审计，不采纳 |
| 条件 OCR rescue / OCR 文本注入 | 否 | 仅以本地/已保存 OCR 证据加强 Indirect | smoke 未达到基线或引入误报，不采纳 |
| MCR | 否 | 对运行时本地 OCR 非空的图像，使用 `untrusted_image_ocr` 结构化上下文；不注入 OCR 文本、不改权重 | 恢复图像/OCR 的角色边界，进入最终策略 |
| SBC | 否 | 仅用 smoke150 锁定图像/OCR 分组 Direct logit penalty `-0.20`；full500 不参与选择 | 与锁定 R0、MCR 共同组成最终策略 |

这里的 R0、MCR、SBC 均是**推理策略**，不修改 LoRA、optimizer、训练数据、checkpoint 或 benchmark 标签。R0 的 `+0.55` 与 SBC 的 `-0.20` 自锁定后保持不变；full500 仅作为盲验收，而不是后续调参数据。

#### 3.4.3 最终 F-3000-MCR-SBC：V2/full500 离线全链路验收

最终部署顺序为：`C5 规则 -> C6B-lite 本地 OCR -> C6A 兼容检查 -> MCR -> F-3000 LoRA head -> R0/SBC -> C4 -> block/allow`。MCR 只依赖本地 OCR 从图像像素读到非空文本这一运行时证据；不读取 benchmark 的 `ocr_text`、`source`、标签或其他元数据。C4 位于 C5/C6 之后，不会绕过已知风险检查。

| 指标 | V2/full500 最终结果 |
|---|---:|
| Accuracy / Macro F1 / Weighted F1 | **62.00% / 59.28% / 61.32%** |
...
| C5 / C6B-lite / head fallback | 35 / 40 / 425 条 |
| MCR 激活 | 40 条 |

500 条 `predictions.jsonl` 完整，包内推理日志无 NaN、Inf 或 traceback。该结果满足三类 recall 非零、Direct F1 >= 35%、Macro F1 >= 45% 的预先声明门槛。正式报告位于 `runs/phase2_3_full_3000_20260729_0958/artifacts/offline_f_3000_mcr_sbc_v2_full500/`；交付审计与 ZIP 位于 `artifacts/delivery/F-3000-MCR-SBC_offline_delivery_v2/` 和同名 `.zip`。

### 3.5 结论、放行状态与后续记录规则

1. Standard Benchmark V1 已正式记录 Balanced-600 与 Full-2000 的历史结果；二者的优化链路均有方向性收益，但均未形成可无条件推广的高可靠三分类器。
2. V2/smoke 的 A-H/F2/F3/F4 是 Full-3000 的训练策略筛选记录，不是最终方案能力结论。它们证明 F 是较平衡的训练起点，也暴露了直接/间接类别边界不足。
3. F 正式训练已完成至 `checkpoint_step_3000`。原始 head 与仅 R0 校准均未通过最终门槛；在冻结 checkpoint 上完成的 MCR + SBC 推理策略通过 smoke 选择后，以 full500 盲验收获得 Macro F1 59.28%、Direct F1 53.56%、Indirect F1 55.90%。
4. 当前最终方案为 **F-3000-MCR-SBC 离线全链路**，不是“重新训练后的 C2B/C3 模型”。后续写入本章的结果必须注明 benchmark、checkpoint、完整 pipeline、锁定推理参数、预测完整性和逐类指标；不得以 full500 结果继续调参。
...
>
> **本部分与前面章节的关系**：
> - **第 1 章**讲"做的过程中碰到的概念是什么"
> - **第 2 章**讲"按什么顺序做"
> - **第 3 章**讲"已经验证出了什么结果"
> - **第 4 章**讲"做完之后还有什么没做、为什么没做、以后怎么补"
...
| 阅读者 | 关心什么 | 本节对应章节 |
|---|---|---|
| **答辩评委** | "这工作有什么不足？未来怎么走？" | §4.2 / §4.7 |
| **你自己（半年后）** | "当时为什么没做 X？现在要不要补？" | §4.3 / §4.4 |
| **合作者 / 接手人** | "接下来能接哪个方向？" | §4.4（路线图） |
| **论文审稿人** | "limitation 部分怎么写" | §4.2 |

**本部分不做什么**：

- ❌ 不重复 Phase 任务（已在第 2 章）
- ❌ 不解释 LoRA / Macro F1 等概念（已在第 1 章）
- ❌ 不写实现细节（已在代码注释）
...
### 4.3 为什么 LoRA 故意只挂语言侧（深度版）

> **这是答辩几乎必问的问题**。§2.E.3 给了技术角度的回答，本节给**研究 + 工程**角度的完整论述。

#### 4.3.1 4 个候选 LoRA 方案对比
...
#### 4.3.5 未来什么时候可以扩到视觉 LoRA

详见 §4.4.2。

---

### 4.4 未来可继续做的工作（3 阶段路线图）
...
#### 4.4.2 中期路线：项目结束后 0.5 ~ 1 年（约 1 年的全职研究投入）
...
#### 4.4.3 长期路线：项目结束后 1 ~ 3 年（约 2-3 年的持续研究投入）
...
- **中期（项目结束后 0.5-1 年）**：用 LLM 生成对抗样本做训练（§4.4.2）→ 让模型学"新模式"
- **长期（项目结束后 1-3 年）**：在线学习（§4.4.3）→ 部署后自动更新
...
### 4.9 项目交付物深度解析：给第三方的不仅是模型

> **本节是项目理解的关键。回答一个常被忽略的问题：项目交付给第三方时，到底交付的是什么？**
> 这一节是第 4 章的"压轴"——前面的 §4.2 讲局限、§4.3 讲设计权衡、§4.4 讲未来；本节讲"交付形态"。
...
#### 4.9.3 项目已设计的应对方案：Phase 2.11 离线包
...
| Phase 6 | 用完整验证集对 `MPID LoRA` vs `C4/C5/C6+MPID LoRA` 做正式对比 | 🟡 中 |
...
这一部分不讲“理想流程”，专门记录项目实施过程中已经沉淀为工程约束的主要问题、返工点和处理原则，方便后来的人直接复用这些经验，而不是重复踩坑。

这一节的重点不是追责，也不记录某次执行的中间失败状态，而是回答 3 个问题：
- 哪类问题会反复影响实施
- 这个问题为什么会发生
- 现在的脚本、参数、目录约定如何规避它
...
| 编号 | 问题 | 发生阶段 | 直接影响 | 稳定处理方式 |
|---|---|---|---|---|
| A1 | mac/MPS 适合开发与 smoke，但不适合作为当前主训练平台 | Phase 0A ~ Phase 2.2 | 长训练速度慢、数值稳定性差 | Windows/x86 作为主执行平台；mac 保留为开发与小规模验证平台 |
| A2 | 小样本直接沿用原始分布会造成类别严重失衡 | Phase 2.2 | 模型偏向多数类，`indirect` 学不起来 | 训练与评估都优先采用分层/均衡抽样，并显式检查 label 分布 |
| A3 | 长时间本地训练必须默认可能被中断 | Phase 2.2 | 训练中断后重跑成本高 | partial checkpoint、周期性保存、resume 参数成为标准流程 |
| A4 | MPS 训练存在 NaN、backward 卡死、OOM 等稳定性风险 | Phase 2 / Phase 2.2 | 即使能开训，也很难稳定跑完 | 使用保守学习率、NaN 防护、mid-epoch save；关键训练放到更稳平台 |
| A5 | 跨平台脚本不能默认 bash 语义 | Phase 0A / Phase 0 / Phase 1 | Windows PowerShell 下路径、编码、系统 API 容易失效 | 文档和脚本显式区分 PowerShell 与 macOS/Linux |
| A6 | 模型/数据下载会受网络抖动影响 | Phase 0A | 环境准备耗时不可控 | 下载脚本支持 retry/resume，准备阶段先做 smoke 再进入训练 |

### 5.3 典型问题详解

#### 5.3.1 A1：mac 起步可行，但不适合作为当前阶段的唯一正式训练平台
...
因此，项目的稳定实施口径是：mac 用于开发、smoke 和小规模验证；长时间训练与端到端验证优先放到更稳定的 x86 Windows 环境。reference 里把 Windows PowerShell 和 macOS/Linux 的命令拆开写，本质上就是这次平台分工留下的工程结果。

#### 5.3.2 A2：小样本训练必须显式控制类别分布

小样本训练如果直接从原始数据分布里截取，很容易放大类别不平衡。主数据集本身就偏向 `direct`，而 `indirect` 是最难、也最关键的一类；如果不给它足够样本，模型会倾向多数类，最终在跨模态/间接注入上表现很弱。

这类分布对 3 分类任务的影响非常直接：模型很容易学成“多数类优先”的分类器。尤其本项目最难、也最关键的一类是 `indirect`，给它的训练样本太少，模型的这部分能力天然就会弱。

稳定处理方式是：训练配置生成阶段就输出 label 分布；小样本训练优先使用 balanced 采样；评估阶段也按 clean/direct/indirect 分层抽样，避免单一总体指标掩盖少数类退化。

#### 5.3.3 A3：长时间训练必须默认可中断、可恢复

Phase 2.2 之后的训练任务动辄数小时，不能再视为“一次性不可中断作业”。机器重启、进程退出、预算到点或人工暂停都应该被视为正常运行条件，而不是异常边界。

稳定处理方式是把恢复能力前置到训练设计中：
- `--save-every` 用于周期性保存 partial checkpoint。
- `--resume-from` 指向已有 checkpoint。
- `--skip-train-batches` / `--resume-global-step` 用于恢复进度与日志口径。
- 长任务必须有固定日志位置和阶段性状态输出。

这个问题最重要的教训不是“机器不要重启”，而是：
- 长时间本地训练必须默认会被中断
- 只在 epoch 边界保存 checkpoint 不够
- 恢复训练必须成为标准流程，而不是临时补救

#### 5.3.4 A4：mac MPS 的问题不只是慢，还有数值和后端稳定性
...
这些问题带来的实际后果是：即使训练“理论上可以开始”，也不适合作为当前阶段最稳的正式训练路径。所以仓库里很多看起来偏保守的设置，其实都是稳定性问题后留下来的防护措施，比如低学习率、NaN skip、防中断保存和 partial checkpoint。

#### 5.3.5 A5：平台迁移不是“换台机器继续跑”，而是一次系统性工程改造
...
从最新仓库形态回看，这些问题已经沉淀成了几类非常具体的工程化结果：
...
5. **把实施问题总结本身当作项目资产**
   reference 记录这些问题不是为了堆历史细节，而是把可复用的工程知识沉淀下来。

## 6. 附录

### 6.A 项目术语速查

> **MPID** = Multimodal Prompt Injection Defense，多模态提示注入防御系统。

> 本节定义 MPID 项目特有的术语体系，确保文档前后一致。

#### 6.A.1 模型阶段命名

| 术语 | 定义 | 说明 |
|---|---|---|
| **Pretrained** | 原生基础模型 | SmolVLM-500M，LoRA 微调前的预训练模型 |
| **MPID LoRA** | 只做 LoRA 微调的模型 | 如 `balanced-600` checkpoint，包含 LoRA 权重 + MPID classification head，不包含 C4/C5/C6 防御层 |
| **MPID LoRA + C4-C6优化** | 包含 C4-C6 优化链路的完整系统 | 在 MPID LoRA 基础上叠加 C4 早退、C5 规则前置、C6 跨模态自检三层防御 |

#### 6.A.2 组件命名

| 术语 | 定义 | 说明 |
|---|---|---|
| **MPID classification head** | 3 分类头 | 用于 `clean / direct / indirect` 三分类的分类层 |
| **C4** | clean 早退机制 | 当 MPID classification head 对 `clean` 给出高置信度时，直接放行 |
| **C5** | 文本规则前置过滤 | 针对直接注入的规则拦截层 |
| **C6** | 跨模态自检 | 针对间接/多模态注入的跨模态一致性检测（含 C6A 启发式） |

#### 6.A.3 评测场景命名

| 术语 | 定义 | 说明 |
|---|---|---|
| **MPID LoRA pipeline** | 仅使用 MPID LoRA 的推理链路 | 不启用 C4/C5/C6 |
| **MPID LoRA + C4-C6优化 pipeline** | 完整防御链路 | `C5 → C6 → MPID classification head → C4 → block/allow → generation` |
| **VLM fallback** | C4/C5/C6 未命中时的回退路径 | 回退到 MPID classification head 进行分类 |

#### 6.A.4 数据与评估术语

| 术语 | 定义 | 说明 |
|---|---|---|
| **balanced-600** | MPID LoRA checkpoint 名称 | 本项目主要测试使用的 checkpoint |
| **clean / direct / indirect** | 三分类标签 | 干净样本 / 直接注入 / 间接注入 |
